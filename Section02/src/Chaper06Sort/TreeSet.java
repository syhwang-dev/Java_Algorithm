...
//		System.out.println("i = " + i + " j = " + j);
	...
	// Union sets with roots i and j, i != j, using the weighting rule.  // WeightedUnion에서 i와 j를 root라고 가정함.
	// parent[i]=-count[i] and parent[j]=-count[j].
	{
		int temp = parent[i] + parent[j];
		if (parent[i] > parent[j]) { // i has fewer nodes  개수를 
			parent[i] = j;
			parent[j] = temp;
...
public class TreeSet {
	public static void main(String[] args) {
		Sets s1 = new Sets(20);
		/*
		// WeightedUnion 할 때 SimpleUnion은 주석
